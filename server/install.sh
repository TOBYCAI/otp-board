...
# After running it, the dashboard is live on http://<host>:3001/.
#
# Interactive: when run in a terminal it asks for domain / admin password /
# refresh interval / token / port / auto-start and shows a confirmation step.
# When piped (curl | bash) it falls back to safe defaults and auto-generates
# tokens, so it still runs unattended.
#
...
echo "============================================"
echo "  OTP Board 服务端 v3.1 一键部署"
echo "  （与原版 otp31.sh 功能一致 + otp-board 兼容）"
echo "============================================"
echo ""

# ----- 升级检测：若目标目录已存在部署，自动读取旧配置作为默认值 -----
# 目的：升级后沿用原管理密码 / 刷新间隔 / 推送 Token / 域名，避免无法登录
# 或手机转发 App 推送被拒(403)。
OLD_PW=""
OLD_DOMAIN=""
OLD_REFRESH=""
...
if [ -f "${INSTALL_DIR}/server/.env" ]; then
  set +e
  OLD_PW=$(grep -E "^ADMIN_PASSWORD=" "${INSTALL_DIR}/server/.env" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '\r')
  OLD_DOMAIN=$(grep -E "^DOMAIN=" "${INSTALL_DIR}/server/.env" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '\r')
  OLD_REFRESH=$(grep -E "^REFRESH_SEC=" "${INSTALL_DIR}/server/.env" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '\r')
  OLD_TOKEN=$(grep -E "^OTP_TOKEN=" "${INSTALL_DIR}/server/.env" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '\r')
  set -e
  if [ -n "${OLD_PW}" ] || [ -n "${OLD_DOMAIN}" ] || [ -n "${OLD_REFRESH}" ] || [ -n "${OLD_TOKEN}" ]; then
    echo "检测到已有部署（${INSTALL_DIR}），将自动沿用以下配置，直接回车即可保留："
    [ -n "${OLD_DOMAIN}" ] && echo "  • 域名: ${OLD_DOMAIN}（回车保留）"
    [ -n "${OLD_PW}" ] && echo "  • 管理密码: 已沿用旧值（回车保留）"
    [ -n "${OLD_REFRESH}" ] && echo "  • 刷新间隔: ${OLD_REFRESH} 秒（回车保留）"
    if [ -n "${OLD_TOKEN}" ]; then
      echo "  • 推送 Token: ${OLD_TOKEN:0:6}********（回车沿用，手机转发 App 无需改动）"
    else
      echo "  • 推送 Token: 旧版未设置；本轮将自动生成（手机 App 需带上）"
    fi
    echo ""
  fi
fi
...
if [ -n "${OLD_DOMAIN}" ]; then
  ask DOMAIN "请输入你的域名（直接回车沿用 ${OLD_DOMAIN}）" "${OLD_DOMAIN}"
elif [ "${INTERACTIVE}" -eq 1 ]; then
  ask DOMAIN "请输入你的域名（格式例如 otp.example.com；用于 WebAuthn/反代）" ""
  while [ -z "${DOMAIN}" ]; do
    ask DOMAIN "域名不能为空，请重新输入" ""
  done
else
  # 非交互（curl | bash）：无历史域名时用 localhost 兜底，避免卡死；部署后可改
  DOMAIN="localhost"
fi

ask ADMIN_PW "请设置管理控制台密码（默认 admin123，回车沿用旧值）" "${OLD_PW:-admin123}"
ask REFRESH_SEC "前台刷新间隔（秒，默认 5，回车沿用旧值）" "${OLD_REFRESH:-5}"
ask OTP_TOKEN "推送接口 Token（留空自动沿用/生成：旧版已有则沿用，无则随机生成）" ""
if [ -z "${OTP_TOKEN}" ]; then
  if [ -n "${OLD_TOKEN}" ]; then
    OTP_TOKEN="${OLD_TOKEN}"
  else
    OTP_TOKEN=$(openssl rand -hex 16 2>/dev/null || (command -v node >/dev/null 2>&1 && node -e "console.log(require('crypto').randomBytes(16).toString('hex'))") || echo "otp$(date +%s%N | md5sum 2>/dev/null | cut -c1-16 || echo token1234567890)")
  fi
fi
ask PORT "HTTP 监听端口" "3001"
...
echo "  4. PM2 自带自启"
echo "============================================"
ask AUTO_START "请选择开机自启方式 (1/2/3/4，默认 3)" "3"

echo "============================================"
echo "  请确认以下信息："
echo "  域名:             ${DOMAIN}"
echo "  管理控制台密码:   ${ADMIN_PW}"
echo "  刷新间隔:         ${REFRESH_SEC}秒"
echo "  监听端口:         ${PORT}"
echo "  推送 Token:       ${OTP_TOKEN}"
case ${AUTO_START} in
  1) echo "  开机自启:         不配置" ;;
  2) echo "  开机自启:         crontab @reboot" ;;
  3) echo "  开机自启:         systemd 服务" ;;
  4) echo "  开机自启:         PM2 自带自启" ;;
esac
echo "============================================"
echo ""
if [ "${INTERACTIVE}" -eq 1 ]; then
...
# ----- 1. Node.js 检查 / 自动安装（>=18）-----
NODE_BIN="$(command -v node || true)"
if [ -z "${NODE_BIN:-}" ]; then
  echo "[1/5] 未检测到 Node.js，尝试自动安装 v20..."
  if command -v apt-get >/dev/null 2>&1; then
    apt-get update -qq 2>/dev/null || true
    apt-get install -y curl gnupg ca-certificates -qq 2>/dev/null || true
    if curl -fsSL https://deb.nodesource.com/setup_20.x | bash - 2>/dev/null; then
      apt-get install -y nodejs -qq 2>/dev/null || true
    fi
  fi
  if ! command -v node >/dev/null 2>&1; then
    echo "尝试二进制安装..."
    cd /tmp
    curl -fsSL -o node20.tar.xz https://nodejs.org/dist/v20.19.0/node-v20.19.0-linux-x64.tar.xz 2>/dev/null || true
    if [ -f node20.tar.xz ]; then
      tar -xf node20.tar.xz -C /usr/local/ --strip-components=1 2>/dev/null || true
      rm -f node20.tar.xz
    fi
  fi
  NODE_BIN="$(command -v node || true)"
  if [ -z "${NODE_BIN:-}" ]; then
    echo "ERROR: 自动安装 Node.js 失败，请手动安装 >=18 后重试（https://nodejs.org）。"
    exit 1
  fi
fi
...
# 写入运行时配置（server.js 通过 loadDotEnv 读取；升级时沿用旧值由上面的向导保证）
cat > server/.env <<EOF
PORT=${PORT}
DOMAIN=${DOMAIN}
...
# ----- 3. 安装依赖（带镜像回退）-----
echo "[3/5] 安装 npm 依赖（express / simplewebauthn / ws / nodemailer）..."
cd server
if ! npm install --no-audit --no-fund --registry=https://registry.npmmirror.com 2>/dev/null; then
  npm install --no-audit --no-fund 2>&1 | tail -3
fi

# ----- 4. 可选：自动安装 PM2（选择 4 且未安装时）-----
if [ "${AUTO_START}" = "4" ] && ! command -v pm2 >/dev/null 2>&1; then
  echo "[4/5] 安装 PM2..."
  npm install -g pm2 --registry=https://registry.npmmirror.com 2>/dev/null || npm install -g pm2 2>&1 | tail -3 || true
fi
...
EOF
        ...
        echo "  未找到 systemctl，跳过开机自启（方式 3 不可用，可改用方式 2）。"
      fi
      ;;
    4)
      if command -v pm2 >/dev/null 2>&1; then
        ...
        echo "  未安装 pm2，跳过开机自启（方式 4 不可用，可改用方式 2）。"
      fi
      ;;
    *) echo "  不配置开机自启（手动启动）。" ;;
  esac
...
if [ "${AUTO_START}" = "3" ] && command -v systemctl >/dev/null 2>&1; then
  systemctl start otp-board >/dev/null 2>&1 || true
  echo "  started via systemd (systemctl start otp-board)."
elif [ "${AUTO_START}" = "4" ] && command -v pm2 >/dev/null 2>&1; then
  pm2 start server.js --name otp-server --update-env 2>/dev/null || pm2 start server.js --name otp-server
  pm2 save 2>/dev/null || true
  echo "  started via pm2 (name: otp-server)."
...
echo ""
echo "============================================"
echo "  OTP Board v3.1 部署完成"
echo "============================================"
echo ""
echo "服务端已启动，监听 0.0.0.0:${PORT}"
echo ""
echo "访问地址:"
echo "  前台看板:    http://<this-host>:${PORT}/"
echo "  管理控制台:  http://<this-host>:${PORT}/admin"
echo "  推送接口:    http://<this-host>:${PORT}/otp"
echo "  健康检查:    http://<this-host>:${PORT}/healthz"
echo ""
echo "生产环境建议用 Nginx/Caddy 做 HTTPS 反代："
echo "  1. 域名 ${DOMAIN} 添加 A 记录指向本机"
echo "  2. 反代目标 http://127.0.0.1:${PORT}（示例见 server/deploy/nginx.example.conf）"
echo "  3. 申请 Let's Encrypt 证书后，前台看板为 https://${DOMAIN}，管理控制台为 https://${DOMAIN}/admin"
echo ""
echo "  ----- 短信转发 App 配置（直接复制到 App 对应输入框）-----"
echo "  接收地址 : http://<this-host>:${PORT}/otp  （生产用 https://${DOMAIN}/otp）"
echo "  Token    : ${OTP_TOKEN}"
echo "  （发送时把 Token 作为 POST 表单字段 token= 或请求头 x-otp-token / x-token 带上即可）"
echo "  -------------------------------------------------------"
echo ""
echo "管理命令:"
echo "  pm2 status              # 查看状态"
echo "  pm2 logs otp-server     # 查看日志"
echo "  pm2 restart otp-server  # 重启服务"
echo ""
case ${AUTO_START} in
  1) echo "开机自启: 未配置，重启后需手动启动" ;;
  2) echo "开机自启: crontab @reboot（重启后自动启动）" ;;
  3) echo "开机自启: systemd（重启后自动启动）" ;;
  4) echo "开机自启: PM2 自带（重启后自动启动）" ;;
esac
echo "============================================"
